...
AS BEGIN TRAN
    ...
    IF EXISTS (SELECT * FROM LICHHEN WHERE MANHASI = @MANHASI AND (ABS(GIO - @GIO) <= 60)) BEGIN
        RAISERROR('NHA SI DA CO LICH GAT GAN KHUNG GIO NAY', 16, 1)
        ROLLBACK TRAN
        RETURN -4
    END
...
AS BEGIN TRAN
    ...
    IF (@MAKHACHHANG IS NOT NULL AND (NOT EXISTS(SELECT * FROM KHACHHANG WHERE MAKHACHHANG = @MAKHACHHANG)))
    BEGIN
        RAISERROR('THONG TIN UPDATE KHACH HANG KHONG HOP LE', 16, 1)
        ROLLBACK TRAN
        RETURN -5
    END
    ...
    IF (@MAKHACHHANG IS NOT NULL) BEGIN
        UPDATE LICHHEN
